...
const TPLS={
  ...
  // Policiales: fondo azul oscuro en banda inferior + acento rojo
  policiales(W,H){const bh=Math.round(H*.34);ctx.fillStyle='rgba(10,20,60,.92)';ctx.fillRect(0,H-bh,W,bh);ctx.fillStyle='#d32f2f';ctx.fillRect(0,H-bh,W,Math.round(H*.022));},
  // Clima: degradado azul cielo de arriba hacia abajo
  clima(W,H){const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'rgba(25,80,160,.55)');g.addColorStop(1,'rgba(5,30,80,.75)');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);},
  // Urgente: banda roja superior + degradado oscuro
  urgente(W,H){ctx.fillStyle='#c62828';ctx.fillRect(0,0,W,Math.round(H*.055));const g=ctx.createLinearGradient(0,H*.4,0,H);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(0,0,0,.88)');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);},
  // Economía: banda verde oscuro inferior
  economia(W,H){const bh=Math.round(H*.32);ctx.fillStyle='rgba(20,60,30,.92)';ctx.fillRect(0,H-bh,W,bh);ctx.fillStyle='#43a047';ctx.fillRect(0,H-bh,W,Math.round(H*.022));},
};
...
function drawPreviewOnCanvas(c,k){
  ...
  const ov={
    ...
    policiales:()=>{tc.fillStyle='rgba(10,20,60,.92)';tc.fillRect(0,H*.66,W,H*.34);tc.fillStyle='#d32f2f';tc.fillRect(0,H*.66,W,H*.03);},
    clima:()=>{const g=tc.createLinearGradient(0,0,0,H);g.addColorStop(0,'rgba(25,80,160,.55)');g.addColorStop(1,'rgba(5,30,80,.75)');tc.fillStyle=g;tc.fillRect(0,0,W,H);},
    urgente:()=>{tc.fillStyle='#c62828';tc.fillRect(0,0,W,H*.07);const g=tc.createLinearGradient(0,H*.4,0,H);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(0,0,0,.88)');tc.fillStyle=g;tc.fillRect(0,0,W,H);},
    economia:()=>{tc.fillStyle='rgba(20,60,30,.92)';tc.fillRect(0,H*.66,W,H*.34);tc.fillStyle='#43a047';tc.fillRect(0,H*.66,W,H*.03);},
  };
  if(ov[k])ov[k]();
